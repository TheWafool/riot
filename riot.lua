local Riot = {RainbowColorValue = 0, HueSelectionPosition = 0}
local PresetColor = Color3.fromRGB(255, 183, 3)
...
local RiotLib = Instance.new("ScreenGui")
RiotLib.Name = "RiotLib"
RiotLib.Parent = game.CoreGui
RiotLib.ZIndexBehavior = Enum.ZIndexBehavior.Sibling

coroutine.wrap(
	function()
		while wait() do
			Riot.RainbowColorValue = Riot.RainbowColorValue + 1 / 255
			Riot.HueSelectionPosition = Riot.HueSelectionPosition + 1

			if Riot.RainbowColorValue >= 1 then
				Riot.RainbowColorValue = 0
			end

			if Riot.HueSelectionPosition == 80 then
				Riot.HueSelectionPosition = 0
			end
		end
	end
)()
...
function Riot:Window(text, bottom,mainclr,toclose)
	CloseBind = toclose or Enum.KeyCode.RightControl
	PresetColor = mainclr or Color3.fromRGB(255, 183, 3)
	local fs = false
		...
		function(io, p)
			if io.KeyCode == CloseBind then
				if uitoggled == false then
					...
					uitoggled = true
					wait(.5)
					RiotLib.Enabled = false
				else
					MainFrame:TweenSize(UDim2.new(0, 706, 0, 484), Enum.EasingDirection.Out, Enum.EasingStyle.Quart, .6, true)
					RiotLib.Enabled = true
					uitoggled = false
				end
			end
		end
	...
	function Riot:Notification(desc,buttontitle)
				...
				function()
					...
					if RainbowColorPicker then
						...
						while RainbowColorPicker do
							BoxColor.BackgroundColor3 = Color3.fromHSV(Riot.RainbowColorValue, 1, 1)
							Color.BackgroundColor3 = Color3.fromHSV(Riot.RainbowColorValue, 1, 1)

							ColorSelection.Position = UDim2.new(1, 0, 0, 0)
							HueSelection.Position = UDim2.new(0.48, 0, 0, Riot.HueSelectionPosition)
